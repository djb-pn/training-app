...
const QuestionEngine = ({ moduleData, title, onBack, accent, userEmail }) => {
  ...
  const handleAnswer = (selected) => {
    ...
    setFeedback({ correct: isCorrect, source: q.source, selected });
  ...
  if (isFinished) {
    return (
      <div style={{ maxWidth: '600px', margin: '100px auto', textAlign: 'center', background: 'white', padding: '50px', borderRadius: '15px' }}>
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '40px auto', padding: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
        ...
        <button onClick={flagQuestion} style={{ color: '#999', background: 'none', border: 'none', cursor: 'pointer' }}>🚩 Flag Error</button>
      </div>
      ...
      <h3 style={{ fontSize: '1.5rem', lineHeight: '1.4' }}>{q.question}</h3>
        ...
        {q.options.map((opt, i) => {
          ...
          return (
            <button 
              ...
              style={{ textAlign: 'left', padding: '20px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '1.1rem', background: bgColor, cursor: feedback ? 'default' : 'pointer' }}>
              {opt}
            </button>
          );
        })}
      ...
      {feedback && (
        <div style={{ marginTop: '30px', padding: '25px', borderRadius: '8px', background: feedback.correct ? '#f0fff4' : '#fff5f5', border: `1px solid ${feedback.correct ? '#c6f6d5' : '#fed7d7'}` }}>
          <h4 style={{ margin: '0 0 10px 0', color: feedback.correct ? '#2f855a' : '#c53030' }}>
            {feedback.correct ? 'Correct!' : 'Incorrect - Shuffling for review.'}
          </h4>
          <p style={{ margin: 0 }}><strong>Source:</strong> {feedback.source}</p>
          <button onClick={nextQuestion} style={{ marginTop: '20px', padding: '10px 25px', background: accent, color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}>Continue</button>
        </div>
      )}
    </div>
  );
