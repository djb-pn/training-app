export const NursePractitionerW2Questions = {
  "Module: Pediatric Wellness & Assessment": {
    "General Principles": [
      {
        "id": 1001,
        "question": "A nurse practitioner is performing a well-child examination on an 18-month-old toddler. According to the recommended sequence, which assessments should be performed LAST? (Select all that apply)",
        "options": ["Heart auscultation", "Inspection of the ears", "Inspection of the mouth/throat", "Abdominal palpation"],
        "answer": ["Inspection of the ears", "Inspection of the mouth/throat"],
        "source": "cite: 311, 616, 789"
      },
      {
        "id": 1002,
        "question": "At what age is it appropriate to begin taking an oral temperature in a cooperative child?",
        "options": ["1 year", "2 years", "3 years", "4 to 5 years"],
        "answer": "4 to 5 years",
        "source": "cite: 270, 750, 1345"
      },
      {
        "id": 1003,
        "question": "Which anthropometric milestone is a standard expectation for a healthy, term infant's weight?",
        "options": ["Double birth weight by 6 months", "Double birth weight by 12 months", "Triple birth weight by 24 months", "Quadruple birth weight by 12 months"],
        "answer": "Double birth weight by 6 months",
        "source": "cite: 561, 756, 982"
      },
      {
        "id": 1004,
        "question": "By what age should a healthy child be walking beyond just 'cruising' along furniture?",
        "options": ["9-11 months", "12-15 months", "18-20 months", "24 months"],
        "answer": "12-15 months",
        "source": "cite: 1183, 2983"
      },
      {
        "id": 1005,
        "question": "When measuring the length of an infant under 2 years of age, which of the following is required for accuracy?",
        "options": ["Standing on a platform scale", "Recumbent position on a measuring board", "Sitting up against a wall-mounted stadiometer", "Measured in the parent's arms"],
        "answer": "Recumbent position on a measuring board",
        "source": "cite: 299, 468, 1371"
      },
      {
        "id": 1006,
        "question": "According to AAP guidelines, the first well-child visit after discharge from the hospital should occur at what age?",
        "options": ["24 hours", "3 to 5 days", "2 weeks", "1 month"],
        "answer": "3 to 5 days",
        "source": "cite: 222, 1302, 3131"
      },
      {
        "id": 1007,
        "question": "When weighing a toddler (age 1-3), what is the appropriate clothing requirement?",
        "options": ["Fully clothed with shoes", "Nude only", "Dry diaper only", "Light indoor clothing without shoes"],
        "answer": "Dry diaper only",
        "source": "cite: 286, 1360, 2762"
      },
      {
        "id": 1008,
        "question": "At what age should a nurse practitioner begin routine annual blood pressure measurements in healthy children?",
        "options": ["Birth", "1 year", "3 years", "5 years"],
        "answer": "3 years",
        "source": "cite: 272, 744, 4326"
      }
    ],
    "Development & Screening": [
      {
        "id": 1009,
        "question": "According to AAP guidelines, formal developmental screening for autism using M-CHAT should be performed at which two ages?",
        "options": ["6 and 12 months", "9 and 30 months", "12 and 18 months", "18 and 24 months"],
        "answer": "9 and 30 months",
        "source": "cite: 320, 1393, 1597"
      },
      {
        "id": 1010,
        "question": "Which innocent heart murmur is best heard at the upper left sternal border between the first two ribs?",
        "options": ["Still murmur", "Pulmonic murmur", "Venous hum", "Mitral stenosis"],
        "answer": "Pulmonic murmur",
        "source": "cite: 827, 1203, 3002"
      },
      {
        "id": 1011,
        "question": "By what age does the respiratory system of a child typically reach full maturity?",
        "options": ["2-3 years", "5-6 years", "8-9 years", "Adolescence"],
        "answer": "8-9 years",
        "source": "cite: 829, 1209, 3009"
      },
      {
        "id": 1012,
        "question": "Which intoeing condition is commonly associated with femoral anteversion?",
        "options": ["Genu varum", "Metatarsus adductus", "Internal tibial torsion", "Intoeing at the hip level"],
        "answer": "Intoeing at the hip level",
        "source": "cite: 823, 1192, 2988"
      },
      {
        "id": 1013,
        "question": "What is the typical age range for the closure of the anterior fontanel?",
        "options": ["0-2 months", "2-6 months", "6-12 months", "12-18 months"],
        "answer": "12-18 months",
        "source": "cite: 812, 1161, 2908"
      },
      {
        "id": 1014,
        "question": "Which of the following is an absolute disqualifier for sports participation in a pediatric athlete?",
        "options": ["Treated asthma", "Acute myocarditis", "Vision corrected to 20/20", "History of controlled seizure disorder"],
        "answer": "Acute myocarditis",
        "source": "cite: 426, 1496, 1676"
      },
      {
        "id": 1015,
        "question": "During a sports physical, which test is used to evaluate the hips, knees, and ankles simultaneously?",
        "options": ["Romberg test", "Duck walk", "Adams forward bend test", "Tandem walking"],
        "answer": "Duck walk",
        "source": "cite: 412, 1479, 1665"
      },
      {
        "id": 1046,
        "question": "Which factors are known to affect body temperature in children? (Select all that apply)",
        "options": ["Activity/Vigorous play", "Autoimmune disorders", "Environmental temperature", "Time of day (Circadian variation)"],
        "answer": ["Activity/Vigorous play", "Autoimmune disorders", "Environmental temperature", "Time of day (Circadian variation)"],
        "source": "cite: 482, 748, 948, 951"
      }
    ]
  },
  "Module: Clinical Reasoning & Diagnostic Logic": {
    "Reasoning Models": [
      {
        "id": 1016,
        "question": "Which type of clinical reasoning is defined as quick, often automatic, and based mainly on pattern recognition?",
        "options": ["Analytical (Type 2)", "Intuitive (Type 1)", "Hypothetico-deductive", "Rule-based logic"],
        "answer": "Intuitive (Type 1)",
        "source": "cite: 91, 3335, 3888"
      },
      {
        "id": 1017,
        "question": "Which diagnostic logic model uses the 'VINDICATE' mnemonic to categorize etiologies like Neoplastic? (Select all that apply)",
        "options": ["Vascular", "Infectious", "Neoplastic", "Autoimmune"],
        "answer": ["Vascular", "Infectious", "Neoplastic", "Autoimmune"],
        "source": "cite: 9, 1744, 3889"
      },
      {
        "id": 1018,
        "question": "Approximately what percentage of a final clinical diagnosis is typically derived from the patient's history?",
        "options": ["25%", "50%", "85%", "95%"],
        "answer": "85%",
        "source": "cite: 1734, 1870, 3405"
      },
      {
        "id": 1019,
        "question": "What is 'Occam's Razor' in the context of diagnostic reasoning?",
        "options": ["The complex explanation is correct", "A single diagnosis should explain all findings", "Miss the common diagnosis", "Order expensive tests first"],
        "answer": "A single diagnosis should explain all findings",
        "source": "cite: 1748, 1885, 2366"
      },
      {
        "id": 1020,
        "question": "Which term describes a preliminary diagnosis waiting for test results, like presumptive strep?",
        "options": ["Final diagnosis", "Working diagnosis", "Differential diagnosis", "Rule-out diagnosis"],
        "answer": "Working diagnosis",
        "source": "cite: 1880, 3429, 3961"
      }
    ],
    "Cognitive Biases": [
      {
        "id": 1021,
        "question": "What is the most common cognitive error in diagnostic reasoning?",
        "options": ["Anchoring", "Availability heuristic", "Premature closure", "Commission bias"],
        "answer": "Premature closure",
        "source": "cite: 4955, 4980"
      },
      {
        "id": 1022,
        "question": "Which bias involves selecting only data that supports a clinician's preconceived diagnosis? (Select all that apply)",
        "options": ["Anchoring heuristic", "Confirmation bias", "Outcome bias", "Selective filtering of evidence"],
        "answer": ["Confirmation bias", "Selective filtering of evidence"],
        "source": "cite: 107, 4954, 5199"
      },
      {
        "id": 1023,
        "question": "The 'Anchoring Heuristic' is best described as:",
        "options": ["Focusing on rare diseases", "Fixating on an initial piece of information", "Delaying a diagnosis", "Generalizing findings"],
        "answer": "Fixating on an initial piece of information",
        "source": "cite: 4954, 4980, 5152"
      },
      {
        "id": 1024,
        "question": "A clinician choosing a benign diagnosis recall safe cases is using which bias?",
        "options": ["Frequency gambling", "Zebra retreat", "Availability heuristic", "Unpacking principle"],
        "answer": "Frequency gambling",
        "source": "cite: 4954"
      },
      {
        "id": 1025,
        "question": "The 'Sutton Slip' refers to the tendency to:",
        "options": ["Forget names", "Focus only on the obvious diagnosis", "Order excessive diagnostics", "Avoid making a diagnosis"],
        "answer": "Focus only on the obvious diagnosis",
        "source": "cite: 4954, 4968"
      },
      {
        "id": 1049,
        "question": "Diagnostic errors most commonly occur due to which factors? (Select all that apply)",
        "options": ["Subjectivity", "Confirmation bias", "Knowledge deficits", "Inexperience"],
        "answer": ["Subjectivity", "Confirmation bias", "Knowledge deficits", "Inexperience"],
        "source": "cite: 106, 109, 3355, 3896"
      }
    ]
  },
  "Module: Documentation & Professional Standards": {
    "SOAP & Health Records": [
      {
        "id": 1026,
        "question": "Patient-reported symptoms using OLD CARTS are documented in which section of a note?",
        "options": ["Subjective", "Objective", "Assessment", "Plan"],
        "answer": "Subjective",
        "source": "cite: 1759, 2154, 3514"
      },
      {
        "id": 1027,
        "question": "Physical examination findings belong in which section? (Select all that apply)",
        "options": ["Subjective", "Objective", "Assessment", "Direct observations from touch"],
        "answer": ["Objective", "Direct observations from touch"],
        "source": "cite: 1760, 2155, 3515"
      },
      {
        "id": 1028,
        "question": "A patient's medical record is primarily considered a:",
        "options": ["Private diary", "Legal document", "Marketing tool", "Billing-only record"],
        "answer": "Legal document",
        "source": "cite: 2138, 3466"
      },
      {
        "id": 1029,
        "question": "The 'Assessment' section of a SOAP note should only include:",
        "options": ["Patient's words", "Every chronic condition", "Problems addressed during specific visit", "Every test performed"],
        "answer": "Problems addressed during specific visit",
        "source": "cite: 3461, 3540"
      },
      {
        "id": 1030,
        "question": "In the APSO note format, which two sections are placed at the beginning?",
        "options": ["Subjective and Objective", "Objective and Plan", "Assessment and Plan", "Subjective and Assessment"],
        "answer": "Assessment and Plan",
        "source": "cite: 2158, 2220"
      },
      {
        "id": 1032,
        "question": "What does a 'negative finding' in documentation mean?",
        "options": ["The patient was uncooperative", "The results were bad", "An expected symptom was absent", "The provider forgot to check"],
        "answer": "An expected symptom was absent",
        "source": "cite: 2447, 3514"
      }
    ],
    "Health Maintenance": [
      {
        "id": 1031,
        "question": "What is the primary goal of the 'Medical Home' model according to the AAP?",
        "options": ["To provide accessible care", "To provide continuous care", "To provide family-centered care", "All of the above"],
        "answer": "All of the above",
        "source": "cite: 207, 1286, 3110"
      }
    ]
  },
  "Module: Physical Examination Techniques": {
    "Methods & Sounds": [
      {
        "id": 1033,
        "question": "In which body system is the sequence of examination altered to (1) Inspection, (2) Auscultation, (3) Palpation, (4) Percussion?",
        "options": ["Thorax", "Heart", "Abdomen", "Neurological"],
        "answer": "Abdomen",
        "source": "cite: 634, 1689, 2860"
      },
      {
        "id": 1034,
        "question": "Which percussion sound is hollow, heard over healthy lung tissue?",
        "options": ["Flatness", "Dullness", "Resonance", "Tympany"],
        "answer": "Resonance",
        "source": "cite: 801, 1122, 2504"
      },
      {
        "id": 1035,
        "question": "The bell of the stethoscope is best suited for hearing which types of sounds? (Select all that apply)",
        "options": ["High-pitched lung sounds", "Low-pitched murmurs", "Extra heart sounds", "Friction rubs"],
        "answer": ["Low-pitched murmurs", "Extra heart sounds"],
        "source": "cite: 691, 1715, 2897"
      },
      {
        "id": 1036,
        "question": "Which position involves the patient on their back with feet in stirrups?",
        "options": ["Sims", "Lithotomy", "Fowler's", "Trendelenburg"],
        "answer": "Lithotomy",
        "source": "cite: 1701, 2473"
      },
      {
        "id": 1037,
        "question": "When assessing for costovertebral angle tenderness, which technique is used?",
        "options": ["Direct percussion", "Indirect percussion", "Blunt percussion", "Pneumatic percussion"],
        "answer": "Blunt percussion",
        "source": "cite: 678, 1117, 2889"
      },
      {
        "id": 1048,
        "question": "Which percussion sounds are correctly matched to their location? (Select all that apply)",
        "options": [
          "Resonance - Normal lung",
          "Dullness - Liver",
          "Tympany - Gastric air bubble",
          "Flatness - Muscle or bone"
        ],
        "answer": ["Resonance - Normal lung", "Dullness - Liver", "Tympany - Gastric air bubble", "Flatness - Muscle or bone"],
        "source": "cite: 801, 1118, 1125, 1836"
      }
    ],
    "Vital Signs Logic": [
      {
        "id": 1039,
        "question": "A blood pressure cuff that is too small for a patient's arm causes what error?",
        "options": ["Falsely low reading", "Falsely high reading", "Accurate systolic, low diastolic", "No error"],
        "answer": "Falsely high reading",
        "source": "cite: 479, 745, 2571"
      },
      {
        "id": 1040,
        "question": "Until what age is it recommended to measure an apical pulse in pediatric patients?",
        "options": ["Under 2 years", "Under 6 years", "Under 10 years", "Only in adolescents"],
        "answer": "Under 6 years",
        "source": "cite: 271, 742, 1346"
      },
      {
        "id": 1041,
        "question": "What triggers the fever response by affecting the hypothalamus? (Select all that apply)",
        "options": ["Antibiotics", "Endogenous pyrogens", "Prostaglandins", "Hypovolemia"],
        "answer": ["Endogenous pyrogens", "Prostaglandins"],
        "source": "cite: 1779, 1909, 1937"
      },
      {
        "id": 1042,
        "question": "Which vital sign measurement is considered the 'fifth vital sign'?",
        "options": ["Pulse Oximetry", "Height", "Pain Assessment", "Head Circumference"],
        "answer": "Pain Assessment",
        "source": "cite: 1776, 1906, 2093"
      }
    ],
    "Geriatrics & Disabilities": [
      {
        "id": 1044,
        "question": "Which tool is specifically designed to assess pain in patients with dementia? (Select all that apply)",
        "options": ["Numeric scale (0-10)", "PAINAD scale", "Nonverbal cue observation", "FACES scale"],
        "answer": ["PAINAD scale", "Nonverbal cue observation"],
        "source": "cite: 1788, 1921"
      }
    ]
  }
};